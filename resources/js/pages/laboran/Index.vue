...
const { data: submissions, loading, execute } = useApi<Pengajuan[]>('/api/laboran/pengajuan');
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-muted-foreground">
          Daftar pengajuan laboratorium yang memerlukan validasi atau penyelesaian oleh Laboran
        </p>
      </div>
    </div>
    ...
    <Card>
      <CardContent class="p-0 sm:p-6">
        <DataTable 
          ...
          :data="submissions" 
          :loading="loading"
          @row-click="(row) => $inertia.visit(`/laboran/${row.id}`)"
        >
